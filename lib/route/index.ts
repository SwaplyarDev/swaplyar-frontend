...
export const createRouteMatchers = (routes: ConfigRoutes, req: NextRequest) => {
  ...
  const match = (routeList: string[]) =>
    routeList.some((route) => pathName.startsWith(route));

  return {
    isPublicRoute: match(publicRoutes),
    isProtectedRoute: match(protectedRoutes),
    isAuthRoute: match(authRoutes),
    isApiRoute: match(apiRoutes),
  };
};
