...
const CerrarSesion = () => {
  ...
  return (
    <div className="flex h-16 w-[500px] items-center justify-center bg-yellow-400">
      <motion.div
        onMouseOver={() => setHoverState(true)}
        onMouseLeave={() => setHoverState(false)}
        initial={{ width: 40, height: 40 }}
        whileHover={{ width: 131, height: 45 }}
        transition={{ duration: 1, ease: 'easeInOut' }}
        // ${hoverState ? 'h-[44px] w-[131px]  border-[1px] border-[#EBE7E0] bg-transparent' : 'h-[40px] w-[40px]'}
        className={`flex origin-center transform items-center justify-center rounded-[50px] hover:border hover:border-[#EBE7E0] hover:bg-transparent`}
      >
        <motion.div
          initial={{ width: 40, height: 40 }}
          whileHover={{ width: 127, height: 40 }}
          transition={{ duration: 1, ease: 'easeInOut' }}
          className={`flex origin-center transform items-center justify-center rounded-[50px] bg-[#EBE7E0] text-lg font-semibold text-[#012A8E]`}
        >
          {/* <button className="flex flex-row items-center gap-2">
            ...
            {hoverState && <motion.p>Salir</motion.p>}
          </button> */}
        </motion.div>
      </motion.div>
    </div>
