...
const CaedAboutUs: React.FC<CaedAboutUsProps> = ({ cardsData }) => {
    ...
    const [flippedCards, setFlippedCards] = useState<boolean[]>(Array(cardsData.length).fill(false));
        ...
        {cardsData.map((card, index) => {
          ...
          return (
            <div
              ...
              `}
            >
              <div className="card w-52 h-64 relative" onClick={() => handleCardFlip(index)}>
                <div className={`card-inner w-full h-full duration-700 ${flippedCards[index] ? 'transform rotate-y-180' : ''}`}>
