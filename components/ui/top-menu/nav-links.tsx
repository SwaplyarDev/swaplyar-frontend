    ...
    <>
      {Links.map((link) => (
        <Link
          ...
          className={`relative m-1 flex h-[48px] items-center gap-2 font-textFont font-light text-buttonsExtraLigth transition-transform duration-300 ease-in-out hover:scale-105 hover:text-buttonsLigth dark:text-buttonsExtraLigthDark dark:hover:text-buttonsLigthDark ${
            pathname === link.href && pathname !== '/auth/login-register'
              ? 'font-semibold text-buttonsLigth after:absolute after:bottom-3 after:left-0 after:h-[1px] after:w-full after:bg-buttonsLigth after:content-[""] dark:text-buttonsLigthDark dark:after:bg-buttonsLigthDark'
              : ''
          } `}
        >
