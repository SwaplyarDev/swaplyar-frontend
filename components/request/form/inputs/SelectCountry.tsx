...
const SelectCountry: React.FC<SelectCodeCountryProps> = ({ setSelectedCodeCountry, errors, blockAll, textColor }) => {
    ...
    <div ref={dropdownRef}>
      ...
      {isOpen && (
        <motion.ul
          ...
          exit={{ opacity: 0, y: -5 }}
          className="scrollable-list-repentance absolute top-16 z-10 mt-2 max-h-64 w-full overflow-y-auto border border-custom-grayD-300 bg-custom-whiteD shadow-lg"
        >
          {countryOptions.map((country, index) => (
            <li
              key={index}
              className={cn(
                'flex cursor-pointer justify-between px-4 py-2 font-textFont text-lightText hover:bg-gray-200',
                {
