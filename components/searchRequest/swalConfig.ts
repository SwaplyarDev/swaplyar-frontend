...
export const showStatusAlert = (statuses: { text: string; icon: string }[], isDark: boolean) => {
  ...
  <div class="w-full overflow-hidden">
    <ul class="flex flex-col items-center justify-center">
      ${statuses
        .map(
          (message, index) => `
          <div class="w-full max-w-[325px] flex items-center flex-col animate-fade-in" style="animation-delay: ${index * 0.5}s">
            <li class="flex gap-[8px] sm:gap-[17px] h-[50px] w-full max-w-[250px] lg:max-w-[400px] items-center sm:max-w-[250px] justify-center">
              ...
              <div class="flex-1 text-sm md:text-md lg:text-lg break-words w-full sm:max-w-[200px]">${message.text}</div>
  ...
  <button id="close-popup-btn-top" class="absolute top-2 right-2 text-white text-xl">
    ...
    customClass: {
      popup: 'w-full max-w-[90%] sm:max-w-[400px] rounded-lg',
    },
    didOpen: () => {
